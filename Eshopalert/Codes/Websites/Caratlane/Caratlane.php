...
$html = curl('http://www.caratlane.com/catalogsearch/result/index/?q='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 

$price = $domxpath->query( ".//span[@class='info']//span" )->item(0)->nodeValue;
$link = $domxpath->query( ".//div[@class='productDetail productDetailRight']//a/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//div[@class='productDetail productDetailRight']//a//img/@title" )->item(0)->nodeValue;
$title = preg_replace("/'/","",$title);

$price = pricergx($price,48);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Caratlane/caratlane.png"></td>
...
$sqlquery = mysql_query("select ProductId from caratlane where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update caratlane SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into caratlane(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('48','$productid','$price','$link','-','$title',now())") or die(mysql_error());
}
