...
echo 'http://shopping.rediff.com/shopping/index.html#!'.$extra_link.'';


echo $html = curl('http://shopping.rediff.com/shopping/index.html#!Samsung+mobiles');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 


echo $price = $domxpath->query( ".//a[@class='bwsURL black f12 bold']" )->item(0)->nodeValue;
echo $link = $domxpath->query( ".//h4[@class='mitemimg_h4_big']//a/@href" )->item(0)->nodeValue;

echo $price = pricergx($price,10);

if($price!=""&&$link!=""){
/*
echo '<table border="0" width="100%" height="20px" bgcolor="#FFFFFF">
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Rediff/rediff.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Delivery in 5 Working days.</td>
...
$newquery = mysql_query("Update flipkart SET Price='$price',Link='$link' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into flipkart(WebsiteId,ProductId,Price,Link,DeliveryTime,Time) values('10','$productid','$price','$link','Delivered in 2-4 business days.',now())") or die(mysql_error());
}
