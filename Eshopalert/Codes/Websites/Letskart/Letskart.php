...
$html = curl('http://www.letskart.com/index.php?route=product/search&filter_name='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 


$price = $domxpath->query( ".//li[@class='listproduct_price']//span" )->item(0)->nodeValue;
$link = $domxpath->query( ".//li[@class='listproduct_title']//a/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//li[@class='listproduct_title']//a" )->item(0)->nodeValue;

$price = pricergx($price,35);

if($price!=""&&$link!=""){

echo '<table border="0" width="100%" height="20px" bgcolor="#FFFFFF">
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Letskart/letskart.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Delivery in 4-6 working days.</td>
...
$sqlquery = mysql_query("select ProductId from letskart where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update letskart SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into letskart(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('35','$productid','$price','$link','Delivery in 4-6 working days.','$title',now())") or die(mysql_error());
}
