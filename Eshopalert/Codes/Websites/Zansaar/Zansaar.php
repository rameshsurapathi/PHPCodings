...
$html = curl('http://www.zansaar.com/search/'.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 


$pricetag = $domxpath->query( ".//li//p" )->item(1)->nodeValue;
$priceexplode = explode(" ",$pricetag);
if($priceexplode[3]!=""){
$price = $priceexplode[3];
}
else{
$price = $priceexplode[1];
}
$link = $domxpath->query( ".//li//h3//a/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//li//h3//a//img/@title" )->item(0)->nodeValue;
$title = preg_replace("/'/","",$title);

$price = pricergx($price,60);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Zansaar/zansaar.png"></td>
...
$sqlquery = mysql_query("select ProductId from zansaar where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update zansaar SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into zansaar(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('60','$productid','$price','$link','3-5 Working Days.','$title',now())") or die(mysql_error());
}
