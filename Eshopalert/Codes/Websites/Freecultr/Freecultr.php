...
$html = curl('http://www.freecultr.com/catalogsearch/result/?q='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
...
$title = $domxpath->query( ".//h2[@class='product-name']//a/@title" )->item(0)->nodeValue;
$title = preg_replace("/'/","",$title);

$price = pricergx($price,57);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Freecultr/freecultr.png"></td>
...
$sqlquery = mysql_query("select ProductId from freecultr where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update freecultr SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into freecultr(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('57','$productid','$price','$link','2-5 Working days.','$title',now())") or die(mysql_error());
}
