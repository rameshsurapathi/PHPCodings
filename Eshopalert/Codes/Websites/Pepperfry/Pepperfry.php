...
$html = curl('http://www.pepperfry.com/catalogsearch/result/?q='.$extra_link.'&cat=');
$dom = new DOMDocument();
@$dom->loadHTML($html);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Pepperfry/pepperfry.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">7 Working Days.</td>
...
$sqlquery = mysql_query("select ProductId from pepperfry where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update pepperfry SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into pepperfry(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('49','$productid','$price','$link','7 Working Days.','$title',now())") or die(mysql_error());
}
