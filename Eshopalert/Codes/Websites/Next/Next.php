...
$html = curl('http://www.next.co.in/catalogsearch/result/?q='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
...
$price = pricergx($price,20);

if($price!=""&&$link!=""){

echo '<table border="0" width="100%" height="20px" bgcolor="#FFFFFF">
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Next/next.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Delivery in 5 - 7 business days.</td>
...
$sqlquery = mysql_query("select ProductId from next where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update next SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into next(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('20','$productid','$price','$link','Delivery in 5 - 7 business days.','$title',now())") or die(mysql_error());
}
