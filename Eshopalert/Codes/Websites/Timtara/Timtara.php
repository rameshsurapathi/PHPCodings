...
$html = curl('http://www.timtara.com/search.php?catLevel=0&category[]=&search_query='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
...
$pricetag = $domxpath->query( ".//div[@class='timtaraprice']" )->item(0)->nodeValue;
$count = substr_count($pricetag, 'Rs.');
if($count==1){
$price = $domxpath->query( ".//div[@class='timtaraprice']//b" )->item(0)->nodeValue;
}
else{
$price = $domxpath->query( ".//div[@class='timtaraprice']//b" )->item(1)->nodeValue;
}
$link = $domxpath->query( ".//div[@class='ProductDetails']//a[@class='TrackLink']/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//div[@class='ProductDetails']//a[@class='TrackLink']" )->item(0)->nodeValue;


$price = pricergx($price,27);

if($price!=""&&$link!=""){

echo '<table border="0" width="100%" height="20px" bgcolor="#FFFFFF">
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Timtara/timtara.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Delivery Within 3 Weeks.</td>
...
$sqlquery = mysql_query("select ProductId from timtara where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update timtara SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into timtara(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('27','$productid','$price','$link','Delivery Within 3 Weeks.','$title',now())") or die(mysql_error());
}
