...
$html = curl('http://www.bestylish.com/catalogsearch/result/?name=Search&q='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
...
$link = $domxpath->query( ".//li[@class='item first']//a/@href" )->item(1)->nodeValue;
$title = $domxpath->query( ".//li[@class='item first']//a/@title" )->item(1)->nodeValue;

$price = pricergx($price,11);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Bestylish/bestylish.png"></td>
...
$sqlquery = mysql_query("select ProductId from bestylish where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update bestylish SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into bestylish(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('11','$productid','$price','$link','Delivered in 2-3 business days.','$title',now())") or die(mysql_error());
}
