...
$html = curl('http://adexmart.com/search?orderby=position&orderway=desc&search_query='.$extra_link.'&submit_search=Search');
$dom = new DOMDocument();
@$dom->loadHTML($html);
...
$link = $domxpath->query( ".//a[@class='product_img_link']/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//a[@class='product_img_link']/@title" )->item(0)->nodeValue;

$price = pricergx2($price,70);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Adexmart/adexmart.png"></td>
...
$sqlquery = mysql_query("select ProductId from adexmart where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update adexmart SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into adexmart(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('70','$productid','$price','$link','2-4 Working days.','$title',now())") or die(mysql_error());
}
