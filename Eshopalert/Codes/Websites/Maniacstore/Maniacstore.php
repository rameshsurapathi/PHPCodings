...
$html = curl('http://letsshop.in/catalogsearch/result/?q='.$extra_link.'&cat=');
$dom = new DOMDocument();
@$dom->loadHTML($html);
...
$link = $domxpath->query( ".//h3[@class='product-name']//a/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//h3[@class='product-name']//a" )->item(0)->nodeValue;

$price = pricergx($price,29);

if($price!=""&&$link!=""){

echo '<table border="0" width="100%" height="20px" bgcolor="#FFFFFF">
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Maniacstore/maniacstore.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Delivery in 5-7 days.</td>
...
$sqlquery = mysql_query("select ProductId from maniacstore where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update maniacstore SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into maniacstore(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('29','$productid','$price','$link','Delivery in 5-7 days.','$title',now())") or die(mysql_error());
}
