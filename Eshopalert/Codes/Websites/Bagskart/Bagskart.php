...
$html = curl('https://www.bagskart.com/advancedsearch/result/?q='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Bagskart/bagskart.png"></td>
...
$sqlquery = mysql_query("select ProductId from bagskart where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update bagskart SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into bagskart(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('56','$productid','$price','$link','2-5 Working days.','$title',now())") or die(mysql_error());
}
