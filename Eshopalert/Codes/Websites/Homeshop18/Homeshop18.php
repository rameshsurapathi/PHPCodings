...
$html = curl('http://www.homeshop18.com/search:'.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 

$price = $domxpath->query( ".//span[@class='product_new_price']" )->item(0)->nodeValue;
$link = $domxpath->query( ".//p[@class='product_title']//a/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//p[@class='product_title']//a/@title" )->item(0)->nodeValue;


$price = pricergx($price,6);

if($price!=""&&$link!=""){

echo '<table border="0" width="100%" height="20px" bgcolor="#FFFFFF">
<tr>
<td width="40%"><img src="Websites/Homeshop18/homeshop18.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Delivered in 10-11 Working Days.</td>
...
$sqlquery = mysql_query("select ProductId from homeshop18 where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update homeshop18 SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into homeshop18(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('6','$productid','$price','$link','Delivered in 10-11 Working Days.','$title',now())") or die(mysql_error());
}
