...
$html = curl('http://ncarry.nimbuzz.com/search.php?q='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 

$price = $domxpath->query( ".//div[@class='rgtCntnr']//h2" )->item(0)->nodeValue;
$link = $domxpath->query( ".//div[@class='cntrCntnr']//a/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//div[@class='cntrCntnr']//a" )->item(0)->nodeValue;
$title = preg_replace("/'/","",$title);

$price = pricergx($price,68);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Ncarrynimbuzz/ncarrynimbuzz.png"></td>
...
$sqlquery = mysql_query("select ProductId from ncarrynimbuzz where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update ncarrynimbuzz SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into ncarrynimbuzz(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('68','$productid','$price','$link','5-7 Working Days.','$title',now())") or die(mysql_error());
}
