...
$html = curl('http://www.bigadda.com/catalogsearch/result/?q='.$extra_link.'&order=relevance&dir=desc');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 

$price = $domxpath->query( ".//span[@class='price']" )->item(0)->nodeValue;
$link = $domxpath->query( ".//div[@class='others-title']//a/@href" )->item(1)->nodeValue;
$title = $domxpath->query( ".//div[@class='others-title']//a/@title" )->item(1)->nodeValue;

$price = pricergx($price,43);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Bitfang/bitfang.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Delivery In 7 - 10 Days.</td>
...
$sqlquery = mysql_query("select ProductId from bitfang where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update bitfang SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into bitfang(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('43','$productid','$price','$link','Delivery In 7 - 10 Days.','$title',now())") or die(mysql_error());
}
