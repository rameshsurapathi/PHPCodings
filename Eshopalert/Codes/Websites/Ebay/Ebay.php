...
$html = curl('http://www.ebay.in/sch/i.html?&_nkw='.$extra_link.'&_sop=3');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 

$price = $domxpath->query( ".//td[@class='prc g-b']" )->item(0)->nodeValue;
$link = $domxpath->query( ".//div[@class='ttl']/a/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//div[@class='ttl']/a/@title" )->item(0)->nodeValue;

$price = pricergx1($price,3);

if($price!=""&&$link!=""){// display the results only if product is found in website

echo '<table border="0" width="100%" height="20px" bgcolor="#FFFFFF">
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Ebay/ebay.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Will usually ship within 5 business days of receiving cleared payment.</td>
...
$sqlquery = mysql_query("select ProductId from ebay where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update ebay SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into ebay(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('3','$productid','$price','$link','Will usually ship within 5 business days of receiving cleared payment.','$title',now())") or die(mysql_error());
}
