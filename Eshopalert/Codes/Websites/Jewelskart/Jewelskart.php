...
$html = curl('https://www.jewelskart.com/advancedsearch/result/?q='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 


$pricetag = $domxpath->query( ".//div[@class='price-box']//div[@class='price-box']" )->item(0)->nodeValue;
$count = substr_count($pricetag, 'Rs.');
if($count==1){
$price = $domxpath->query( ".//div[@class='price-box']//div[@class='price-box']//span[@class='price']" )->item(0)->nodeValue;
}
else{
$price = $domxpath->query( ".//div[@class='price-box']//div[@class='price-box']//span[@class='price']" )->item(1)->nodeValue;
}
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Jewelskart/jewelskart.png"></td>
...
$sqlquery = mysql_query("select ProductId from jewelskart where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update jewelskart SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into jewelskart(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('55','$productid','$price','$link','2-5 Working days.','$title',now())") or die(mysql_error());
}
