...
$html = curl('http://www.giftcart.com/catalogsearch/result/?order=relevance&dir=desc&q='.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 

$pricetag = $domxpath->query( ".//div[@class='product_price']" )->item(0)->nodeValue;
$count = substr_count($pricetag, 'Rs.');
if($count==1){
$price = $domxpath->query( ".//div[@class='product_price']" )->item(0)->nodeValue;
}
else{
$priceexplode = explode("Rs.",$pricetag);
$price = $priceexplode[1];
}
$link = $domxpath->query( ".//div[@class='product_name']//a/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//div[@class='product_name']//a/@title" )->item(0)->nodeValue;
$title = preg_replace("/'/","",$title);

$price = pricergx($price,61);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Giftcart/giftcart.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">4-6 Working days.</td>
...
$sqlquery = mysql_query("select ProductId from giftcart where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update giftcart SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into giftcart(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('61','$productid','$price','$link','4-6 Working days.','$title',now())") or die(mysql_error());
}
