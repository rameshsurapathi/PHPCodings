...
$html = curl('http://www.buytheprice.com/search__'.$extra_link.'');
$dom = new DOMDocument();
@$dom->loadHTML($html);
$domxpath = new DOMXPath($dom); 

$price = $domxpath->query( ".//div[@class='btn btn-warning']" )->item(0)->nodeValue;
$link = $domxpath->query( ".//a[@class='mosaic-backdrop']/@href" )->item(0)->nodeValue;
$title = $domxpath->query( ".//h2[@class='product-name']/@title" )->item(0)->nodeValue;

$price = pricergx($price,12);
...
<tr>
<td width="40%" valign="middle" style="color:grey;"><img src="Websites/Buytheprice/buytheprice.png"></td>
...
<td width="20%" align="left" style="font-size:12px;">Delivery In 5-7 working days.</td>
...
$sqlquery = mysql_query("select ProductId from buytheprice where ProductId='$productid'") or die(mysql_error());
...
$newquery = mysql_query("Update buytheprice SET Price='$price',Link='$link',Title='$title' where ProductId='$productid'") or die(mysql_error());
}

else{ // Insert product details into website table for first time
$newquery = mysql_query("Insert into buytheprice(WebsiteId,ProductId,Price,Link,DeliveryTime,Title,Time) values('12','$productid','$price','$link','Delivery In 5-7 working days.','$title',now())") or die(mysql_error());
}
